...
<div class="flex row">
    ...
    <div class="w-80 p-4 flex-none">
        <form name='filters' id="filters" action="<?php echo  htmlspecialchars($_SERVER["PHP_SELF"]);?>">
            <div class="pb-3">

                <!-- Hidden input field to hold the value for Category and Sub category -->
                <input type='number' class='hidden' name="catid" value="<?php echo (!is_null($cat_id)) ? $cat_id: '0' ?>" />
                <input type='number' class='hidden' name='subcatid' value="<?php echo (!is_null($subcat_id)) ? $subcat_id: '0'?>" />
                ...
                ?>
                        ...
                        <?php
                            if (!is_null($cat_id) && is_null($subcat_id)){
                                $rows_q = 'SELECT * FROM subcategories WHERE catid=' . $cat_id;
                                $subcat_list = $conn->query($rows_q);
                                if ($subcat_list->num_rows > 0) {
                                    while($row = $subcat_list->fetch_assoc()) {
                                        echo '<button type="button" onclick=updatefilter("subcatid",'. $row['subcatid'] . ')>';
                                ...
                                if ($cat_list->num_rows > 0) {
                                    while($row = $cat_list->fetch_assoc()) {
                                        echo '<span class="pl-4 font-lg">';
                                        echo '<button type="button" onclick="updatefilter( `catid` , ' . $row['catid'] .  ' )" >';
                                        echo $row['catname'];
                                        echo '</button>';
                                        echo '</span>';
                                        echo '<br>';
            ...
            <div class="flex flex-col">
                <div>
                    <button type="button" class="w-full text-xl font-semibold text-left border-none accordion pb-2">Sort By</button>
                    <div class="panel">
                        <input type="radio" id="price_h_l_b" name="sortby" value="price_h_l" <?php if($sortby == 'price_h_l') echo 'checked' ?> class='inline rounded' onchange="this.form.submit();"/>
                        <label for="price_h_l_b">Price (High to Low)</label> <br>
                        <input type="radio" id="price_l_h_b" name="sortby" value="price_l_h" <?php if($sortby == 'price_l_h') echo 'checked' ?> class='inline' onchange="this.form.submit();"/>
                        <label for="price_l_h_b">Price (High to Low)</label><br>
                        <input type="radio" id="rating_b" name="sortby" value="rating" <?php if($sortby == 'rating') echo 'checked' ?> class='inline' onchange="this.form.submit();"/>
                        <label for="rating_b">Rating</label>
                    </div>
                    <hr class="mt-4 border border-gray-200 w-full">
                </div>
                <div>
                    <button type="button" class="w-full text-xl font-semibold text-left border-none accordion pb-2">Price</button>
                    <div class="panel">
                        <div class="mt-6 mb-2" id="price_range"></div>
                        <div class="flex flex-row w-full justify-between">
                            <input type='text' name='min_limit' value="<?php echo isset($min_limit) ? $min_limit : '' ?>" class="text-black w-10 border-none focus:outline-none" readonly />
                            <input type='text' name='max_limit' value="<?php echo isset($max_limit) ? $max_limit : '' ?>" class="text-black w-10 border-none focus:outline-none" readonly />
                        </div>
                    </div>
                    <hr class="mt-4 border border-gray-200 w-full">
                </div>
    ...
    <div class='py-2 pr-2 w-full'>
        ...
        <div class="grid grid-cols-4 gap-4 pt-2">
            ...
            ?>
                        <div class='w-full' onclick=location.href="product.php?productid=<?php echo $row['product_id'] ?>">
                            <img src="<?php echo $row['product_main_image'] ?>">
                            <div class="pt-5">
                                <p class='text-md font-semibold block hover:text-blue-500'><?php echo $row['product_full_name'] ?></p>
                                <?php 
                                ...
                                ?>
                                <p class="text-xl">$<?php echo $row['product_price'] ?></p>
                            </div>
                    </div> 
            <?php
                    }
