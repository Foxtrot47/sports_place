...
(function (factory) {
  ...
  Spectrum.prototype.getDistance = function (value) {
    ...
    for (index = 0; index < this.xNumSteps.length - 1; index++) {
      ...
      if (step && (value / step) % 1 !== 0) {
        throw new Error(
          `noUiSlider (${VERSION}): 'limit', 'margin' and 'padding' of ${this.xPct[index]}% range must be divisible by step.`
        );
      }
  ...
  function scope(target, options, originalOptions) {
    ...
    function addOrigin(base, handleNumber) {
      ...
      if (options.keyboardSupport) {
        ...
        handle.addEventListener("keydown", (event) =>
          eventKeydown(event, handleNumber)
        );
      }
    ...
    function fixEvent(e, pageOffset, eventTarget) {
      ...
      if (touch) {
        ...
        if (e.type === "touchstart") {
          const targetTouches = Array.prototype.filter.call(
            e.touches,
            isTouchOnTarget
          );
          ...
          const targetTouch = Array.prototype.find.call(
            e.changedTouches,
            isTouchOnTarget
          );
    ...
    function updateConnect(index) {
      ...
      const translateRule = `translate(${inRuleOrder(
        `${transformDirection(l, connectWidth)}%`,
        "0"
      )})`;
      const scaleRule = `scale(${inRuleOrder(connectWidth / 100, "1")})`;
      scope_Connects[index].style[
