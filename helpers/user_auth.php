...
// Check if atleast 1 result exist
if ($res->num_rows > 0) {
  ...
  // Send session token for autologin purposes
  //Generate a random string.
  $token = openssl_random_pseudo_bytes(16);
  //Convert the binary data into hexadecimal representation.
  $token = bin2hex($token);
  //Store it in db
  $sql2 = "UPDATE users SET session_token ='{$token}' WHERE user_email='{$email}'";
  $conn->query($sql2);

  // Resending query because 2nd query causes 1st query result to be nulled
  $res = $conn->query($sql);
  
  while ($row = $res->fetch_assoc()) {
    // Send all data to client side , so that they don't need to always make another request
    $out = array('user_id'=> $row["user_id"] ,'user_email'=> $row["user_email"],'first_name'=> $row["first_name"],'last_name'=> $row["last_name"],'user_session_token'=> $token) ;
...
// If user is trying to make a new account
  ...
  $sql = "INSERT INTO users (user_email,first_name,last_name,user_password) 
          VALUES (' {$email} ',' {$first_name} ','{$last_name} ','{$hashed_pass} ')";
  echo $sql;
